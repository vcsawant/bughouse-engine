...
- Static exchange evaluation (SEE): the classic swap algorithm with x-ray support (slider attacks recomputed as occupancy empties) computes the material outcome of a full capture-recapture sequence without searching it. Used to prune losing captures (SEE < 0) from quiescence — roughly halves total search nodes on tactical positions. Check evasions are never pruned.
