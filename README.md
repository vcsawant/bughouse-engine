...
- Static exchange evaluation (SEE): answers "does this capture win material after all recaptures?" without searching. Both sides alternately capture on the square — always with their cheapest attacker, and either side may stop when continuing loses material — resolved as a small backward minimax over the "swap list." Removing used attackers from an occupancy bitboard and recomputing slider attacks exposes x-ray backup (Rxd5 backed by a rook behind it scores +100, not −400). Used to prune losing captures (SEE < 0) from quiescence — roughly halves total search nodes on tactical positions; check evasions are never pruned.
